...
mbart_tokenizer = MBart50TokenizerFast.from_pretrained(MBART_DIR)
...
def validate_lang(lang: str) -> None:
    if lang not in MBART_LANG_CODES:
        raise ValueError(f"Language must be one of {sorted(MBART_LANG_CODES.keys())}")


def generate_mbart_summary(
    text: str,
    src_lang: str,
    ...
    src_code = MBART_LANG_CODES[src_lang]
    ...
    with torch.no_grad():
        output_ids = mbart_model.generate(
            **inputs,
            ...
            num_beams=num_beams,
            forced_bos_token_id=mbart_tokenizer.lang_code_to_id[src_code],
        )

    return mbart_tokenizer.decode(output_ids[0], skip_special_tokens=True)
